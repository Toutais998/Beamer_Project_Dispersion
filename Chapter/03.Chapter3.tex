...
\begin{frame}{RMS 波前误差：文献重建与优化结果对比}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=.485\textwidth,height=.33\textheight,keepaspectratio]{Figure/ZemaxSimulation/RMS_01.png}

        \hspace{0.01\textwidth}

        \includegraphics[width=.485\textwidth,height=.33\textheight,keepaspectratio]{Figure/ZemaxSimulation/RMS_02.png}
        \caption{左：文献参数重建模型的 RMS 波前误差；右：优化后模型的 RMS 波前误差}
    ...
    \begin{columns}[T]
        \begin{column}{.52\textwidth}
            \begin{itemize}
                \item 文献给出的参考判据表明，当 RMS wavefront error 低于约 0.072 waves 时，系统可近似视作接近衍射极限。
                \item 因此，文献参数重建结果可以作为比较基准，用于观察原始设计在主工作视场中的误差抬升趋势。
            \end{itemize}
        \end{column}
        \begin{column}{.48\textwidth}
            \begin{itemize}
                \item 优化后，主要工作波段的 RMS 水平进一步降低，且随视场增大的变化更平缓。
                \item 这表明优化不仅改善了轴上像点，也提高了扫描过程中的全场像质一致性。
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{RMS 与 OTF 的全视场表现}
    ...
    \begin{itemize}
        ...
        \item 这说明系统在边场扫描时仍能维持稳定的低频结构传递能力，与前一页的波前误差结果相互印证。
    \end{itemize}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{.495\textwidth}
            \begin{figure}
                ...
                \caption{文献参数重建模型的网格畸变，虚线框对应标准 $\pm 3^\circ$ 工作视场}
        ...
        \begin{column}{.495\textwidth}
            \begin{figure}
                ...
                \caption{优化后模型在 $\pm 5^\circ$ 扩展视场下的网格畸变}
...
\begin{frame}{Strehl 比：主工作波段的衍射极限保持能力}
    ...
    \begin{columns}[T]
        \begin{column}{.39\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=\textwidth,height=.48\textheight,keepaspectratio]{Figure/ZemaxSimulation/斯特列尔比_01.png}
        ...
        \begin{column}{.39\textwidth}
            \begin{figure}
                \centering
                \includegraphics[width=\textwidth,height=.48\textheight,keepaspectratio]{Figure/ZemaxSimulation/斯特列尔比_02.png}
                \caption{优化后模型在主工作波段的 Strehl 分布}
