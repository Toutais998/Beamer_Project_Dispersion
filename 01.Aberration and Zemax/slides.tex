% \documentclass[10pt,aspectratio=43,table]{beamer} %这个是4：3的
\documentclass[8pt,aspectratio=169,table]{beamer}  %这个是16：9的
...
\setmonofont{Courier New} %等宽字体，用于 \texttt 和 listings 代码块

\usetheme{Berlin} %主题
\usecolortheme{sustech} %主题颜色
\usefonttheme[onlymath]{serif} %替代已废弃的 mathserif 选项
...
% --- 强制正文缩小一档（保持 documentclass 8pt，不影响页眉页脚） ---
\setbeamerfont{normal text}{size=\fontsize{7pt}{9pt}}
...
\usepackage{fancybox}
\usepackage{xcolor}
% \usepackage{times} 已删除：fontspec 已设置 Times New Roman，且 times 包会覆盖等宽字体导致 pcr 警告
\usepackage{listings}
\usepackage{booktabs}
\usepackage{colortbl}
\usepackage{multirow}
